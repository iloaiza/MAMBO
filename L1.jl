...
if SAVING && isfile(FILENAME*".h5")
	println("Loading molecular data from $FILENAME")
	...
	if SAVING
		println("""Saving molecular data in $FILENAME under group "MOLECULAR_DATA". """)
		fid = h5open(FILENAME*".h5", "cw")
		if haskey(fid, "MOLECULAR_DATA")
			@warn "Trying to save molecular data to $FILENAME, but MOLECULAR_DATA group already exists."
			close(fid)
			oldfile(FILENAME)
			fid = h5open(FILENAME*".h5", "cw")
		end
